...
const ENABLE_TEMP_COMMENT_DIVERSITY_GUARD = true;
const TEMP_COMMENT_DIVERSITY_MAX_RETRIES = 2;
const TEMP_COMMENT_SIMILARITY_THRESHOLD = 0.72;
...
function calcJaccardSimilarity(a: string, b: string): number {
...
function calcMaxSimilarity(candidate: string, existingComments: string[]): number {
    if (existingComments.length === 0) return 0;
    return existingComments.reduce((max, comment) =>
        Math.max(max, calcJaccardSimilarity(candidate, comment)), 0);
}

function hasSimilarPrefix(candidate: string, existingComments: string[]): boolean {
    const prefixLen = 10;
    const normalizedCandidate = normalizeForSimilarity(candidate).slice(0, prefixLen);
    if (!normalizedCandidate) return false;

    return existingComments.some((comment) =>
        normalizeForSimilarity(comment).slice(0, prefixLen) === normalizedCandidate);
}
    ...
    try {
        ...
        try {
            ...
            if (!existingCommentsSnapshot.empty) {
                ...
                existingAICommentTexts = existingCommentsSnapshot.docs
                    .map((doc) => String(doc.data().content || "").trim())
...
`;
        ...
        const commentsForSimilarityCheck = existingAICommentTexts.slice(-6);

        for (let attempt = 0; attempt <= TEMP_COMMENT_DIVERSITY_MAX_RETRIES; attempt++) {
            const aiResult = await aiFactory.generateText(
                activePrompt,
                attempt > 0 ? { temperature: 0.95 } : undefined
            );
            ...
            if (attempt >= TEMP_COMMENT_DIVERSITY_MAX_RETRIES) {
                console.warn("[DIVERSITY GUARD] Max retries reached. Skip similar AI comment.");
                commentText = "SKIP_COMMENT";
                break;
            }

            activePrompt = `${prompt}

# Diversity Retry Rule
- 既存コメントと文頭・表現が重複しない別切り口で書いてください。
- 「すごい」「素敵」「応援してる」等の定型褒め語だけで構成しないでください。
- 具体的に投稿内容の別ポイントを1つだけ選んで一文で返してください。`;
        }
        ...
        console.error("Error in generateAIComment:", summarizeErrorForLog(error));
...
export const generateAIReactionV1 = functionsV1.region(LOCATION).https.onRequest(async (request, response) => {
    ...
    try {
        ...
        console.error("Error in generateAIReaction:", summarizeErrorForLog(error));
        response.status(500).send("Internal Server Error");
    }
});
...
export const executeAIPostGeneration = functionsV1.region(LOCATION).runWith({
    ...
    try {
        ...
        const persona = AI_PERSONAS.find((p) => p.id === personaId);
        if (!persona) {
            console.error("Persona not found for AI post generation", { personaId });
            response.status(400).send("Persona not found");
            return;
        }
