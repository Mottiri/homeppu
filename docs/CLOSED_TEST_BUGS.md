...
### CT-017: 低速端末でチュートリアルスポットライト調整がカクつく

**報告日**: 2026-03-06
**優先度**: 中
**画面**: チュートリアル / プロフィール
**ステータス**: 保留（低速端末実機確認で許容範囲と判断、2026-03-11）

**問題**:
低速端末で、チュートリアル中のスポットライト位置調整や自動スクロール時に動作がカクつく。特にプロフィール画面の Phase6（徳ポイント / お気に入り説明）で、スポットライト再計算と背景スクロールが重なる際に負荷が高くなりやすい。

**タスク**:
- [ ] 低速端末でのチュートリアル挙動を再計測し、重いフレームを特定する
- [ ] `build` 中の post-frame callback / Rect 再計算の回数を削減する
- [ ] スポットライト位置確定後は不要な再計算を止める
- [ ] 自動スクロールとスポットライト再表示のタイミングを最適化する

**関連ファイル**:
- `lib/features/profile/presentation/screens/profile_screen.dart`
- `lib/shared/widgets/tutorial_overlay.dart`

**備考**: アプリ全体のパフォーマンス最適化は `docs/specs/2026-03-10_performance_optimization.md` に分離。本チケットはチュートリアル起因の症状に限定。性能改善 #2・#6・#8・#9 の実装後、低速端末での実機確認でアプリ全体のパフォーマンスが許容範囲と判断されたため、本チケット含む残りの性能改善項目は一括で保留とした（2026-03-11）。再開条件は性能改善設計書を参照。

---

### CT-018: アプリ全体のスケーラビリティ改善

**報告日**: 2026-03-11
**優先度**: 高
**カテゴリ**: インフラ / スケーラビリティ
**ステータス**: 調査完了 → 改善対応中（P0+P1 全6項目）

**問題**:
アプリ全体の現在の機能・構成・サーバー設計を見たとき、ユーザー規模が拡大した際に耐えうる設計となっているか不明。事前に調査し、ボトルネックやスケーリング上の課題を洗い出す必要がある。

**調査結果**: 9項目のリスクを特定。P0（3項目）+P1（3項目）の計6項目を対応中。
**管理フォルダ**: `docs/scalability/` で設計書・進捗を一元管理

**調査観点**:
- [ ] Firestore クエリ設計（インデックス、読み取りコスト、N+1パターン）
- [ ] Cloud Functions のスケーリング特性（コールドスタート、同時実行数、タイムアウト）
- [ ] Cloud Tasks / スケジュール関数の処理容量
- [ ] Firebase Authentication の同時接続数制限
- [ ] Firebase Storage の帯域・アクセスパターン
- [ ] AI機能の API レート制限・コスト（Gemini / OpenAI）
- [ ] リアルタイムリスナー（Firestore streams）の接続数上限
- [ ] Follow Feed のクエリ方式（現行 Chunked Query → Fan-out 移行判断）
- [ ] プッシュ通知の配信スケーラビリティ
- [ ] 広告配信のパフォーマンスへの影響
- [ ] データモデル設計（コレクション構造、ドキュメントサイズ上限）
- [ ] コスト見積もり（ユーザー100人 / 1,000人 / 10,000人 / 100,000人規模）

**成果物**:
- スケーラビリティ調査報告書（`docs/specs/` に保存）
- ボトルネック一覧と対策の優先度付けリスト
...
**問題**:
一部のステップでスポットライトの位置が対象ウィジェットとずれていた。

**解決内容**:
- [x] 各ステップのスポットライト矩形計算を確認
- [x] キータイミングとオフセット計算を修正
- [x] プロフィール画面の徳ポイント / お気に入り説明のスポット対象を見直し
- [x] 必要な箇所で背景のみ自動スクロールしてスポットが見える状態に調整

**修正**:
スポットライト矩形の取得方法を見直し、オフセット補正のばらつきを縮小。プロフィール画面では徳ポイント領域・お気に入りタブのスポット対象を調整し、チュートリアルUIと重なる場合はキャラ位置を固定したまま背景側のみスクロールしてスポットが見える状態に揃えた。

**補足**:
低速端末でのカクつきは別件として `CT-017` に分離。
...
- `lib/features/stamps/presentation/screens/stamp_sheet_screen.dart`
...
**解決内容**:
- [x] コメントのいいね表示箇所を特定
- [x] いいね済み状態のUI表示（テキスト・アイコン変化等）を非表示化
- [x] いいね機能自体（データ保存・通知等）は維持
- [x] 投稿主だけに「いいね！しました」をコメントUI内で表示

**修正**:
コメントカード下部の公開いいね表示を削除し、投稿主が長押しでいいねした場合のみ、その投稿主にだけコメントUI内で「いいね！しました」を表示するよう変更。通知や保存データはそのまま維持し、第三者からはいいね有無が見えない状態に統一。

**関連ファイル**:
- `lib/features/post/presentation/screens/post_detail_screen.dart`
- `lib/core/constants/app_messages.dart`

### CT-016: AIモード・ミックスモードのAI返信に関する説明不足
...
**問題**:
公開範囲が「AIモード」「ミックスモード」の場合、AIからコメントやリアクションが届くことの説明が不足していた。また、AI生成コンテンツは内容が不正確になる可能性があるが、その注意事項の記載がなかった。

- 設定画面のAIモード説明: AIから返信が来ることの明記なし
- 設定画面のミックスモード説明: AIからも返信が来ることの明記なし
- チュートリアル: AIモードのみ「AIからの返信はくるよ」と記載、ミックスモードは未記載
- AI生成の注意事項（不正確な場合がある等）: どこにも記載なし

**解決内容**:
- [x] 設定画面のモード説明にAIからコメント・リアクションが届く旨を追記（AIモード・ミックス）
- [x] チュートリアルの各モード説明を統一・強化
- [x] チュートリアル完了ステップにAI注意事項を追記
- [x] 「アプリについて」画面に「AIについて」セクションを新規追加
- [x] ヘルプ（HELP.md）に「公開範囲とAIについて」セクションを新規追加

**関連ファイル**:
- `lib/features/profile/presentation/screens/settings_screen.dart`（PrivacyMode enum）
- `lib/core/constants/app_messages.dart`（チュートリアル・アプリ情報メッセージ）
- `lib/features/profile/presentation/screens/about_app_screen.dart`（AIセクション追加）
- `docs/HELP.md`（公開範囲とAIセクション追加）
