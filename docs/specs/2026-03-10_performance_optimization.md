...
**ステータス**: #2 実装完了、#1/#3〜#7 設計中
**次のアクション担当者**: 実装者 → #6 Phase1 Chunked Query に着手
...
### Phase 1: Chunked Query（今回のスコープ）

`whereIn` の10件制限を、クライアント側でクエリ分割して対応する。

- [ ] `followingIds` を10件ずつチャンクに分割
- [ ] 各チャンクで並列クエリを実行
- [ ] 結果をクライアント側で `createdAt` 順にマージ
- [ ] ページネーションとの整合性を確認
