...
#### 8. リアクション選択シートのタブ化
- **ファイル**: 
  - `lib/features/home/presentation/widgets/reaction_selection_sheet.dart` - タブ形式に変更
  - `lib/shared/services/recent_reactions_service.dart` - 新規作成
  - `lib/features/home/presentation/widgets/reaction_button.dart` - 直近使用リスト更新処理追加
- 画面いっぱいではなく固定高さ（320px）でコンパクト表示
- タブでカテゴリ切り替え（スワイプ対応）
- 「よく使う」タブ（直近使用した最大7件）を先頭に表示

#### 9. 月間カレンダーの完了タスク優先非表示
- **ファイル**: `lib/features/tasks/presentation/screens/monthly_calendar_screen.dart`
- 複数タスクがある日で省略表示される際、完了済みタスクを優先的に非表示
- 未完了タスクを優先表示、同じ完了状態なら優先度で降順ソート

#### 10. AIリアクション数の調整
- **ファイル**: `functions/src/index.ts` - `onPostCreated`関数
- リアクション数を15〜30件から5〜15件（平均10件）に変更
- 通知過多を解消し、MIXモードへの誘導を促進

#### 11. サークル画像設定（アイコン・ヘッダー）
- **ファイル**: 
  - `lib/features/circle/presentation/screens/edit_circle_screen.dart` - 画像設定UI追加
  - `lib/features/circle/presentation/screens/create_circle_screen.dart` - 画像設定UI追加
  - `lib/shared/services/media_service.dart` - `uploadCircleImage`メソッド追加
  - `lib/shared/services/image_moderation_service.dart` - 画像モデレーションサービス新規作成
  - `functions/src/index.ts` - `moderateImageCallable` Callable関数追加
  - `firebase/storage.rules` - circles/{circleId}/{imageType}/{fileName}ルール追加
  - `android/app/src/main/AndroidManifest.xml` - UCropActivity追加
- アイコン（1:1）・ヘッダー（16:9）のアスペクト比固定クロッピング
- 削除ボタン（×）でデフォルトに戻る機能
- `image_cropper`パッケージで画像調整UI提供
- **AIモデレーション**: 保存ボタン押下時にGemini APIで画像チェック、不適切ならブロック
