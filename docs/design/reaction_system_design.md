...
### C. リアクション選択シート（旧）
*   **表示形式**: モーダルボトムシート。
*   ~~廃止予定: 長押しオーバーレイUIに置き換え~~

### D. 長押しリアクションUI（新）
*   **表示形式**: 投稿カード長押しでオーバーレイ表示。
*   **UI要素**:
    *   **暗い背景**: 画面全体に60%透明度の黒背景。
    *   **スタンプバー**: ボトムナビの上（bottom: 100px）に中央配置、**横スクロール可能**。
    *   **構成**: 5つのスタンプ + ＋アイコン（右端）。
    *   **＋アイコン押下**: 右側に縦長リストで残りスタンプを表示。
    *   **背景タップで閉じる**: barrierDismissible。
*   **アニメーション**:
    *   **スタガードアニメーション**: 各スタンプが80msずつ遅延して順番に出現。
    *   **ポップ効果**: 0 → 1.2倍 → 1.0倍のバウンドアニメーション。
    *   **拡張リスト**: TweenAnimationBuilder + elasticOutカーブでポップ表示。
*   **スタンプ昇格ロジック**:
    *   使用したスタンプは`RecentReactionsService`に記録。
    *   次回オーバーレイ表示時、**最近使用した順**でスタンプを並び替え。
    *   最も最近使用したスタンプがバーの左端に表示される。
*   **制約**:
    *   自分の投稿にはリアクション不可。
    *   1回の表示で**最大5回**までリアクション可能（5回で自動閉じ）。
*   **カスタムアセット（7種類）**:
    | 名前 | ファイル | 絵文字 | ラベル |
    |------|----------|--------|--------|
    | heart | heart.png | ❤️ | ハート |
    | praise | praise.png | ✨ | すごい |
    | shine | shine.png | 💫 | キラキラ |
    | clap | clap.png | 👏 | 拍手 |
    | star | star.png | ⭐ | スター |
    | hundred | hundred.png | 💯 | 100点 |
    | rainbow | rainbow.png | 🌈 | にじ |
    *   配置: `assets/reactions/{value}.png`
...
### 概要
ユーザーが独自のスタンプ画像を作成・登録し、自分の投稿へのリアクションに使用できる機能。

### 仕様
*   **使用範囲**: 作成者本人のみ使用可能（他ユーザーとの共有なし）
*   **ファイル制限**: サイズ上限あり（例: 500KB）
*   **保存先**: Firebase Storage + Firestore

### 審査フロー
1.  **システム審査**: 自動で明らかな違反画像を拒否（AI画像審査）
2.  **管理者審査**: システム審査通過後、管理者が目視で許可/拒否
3.  **通知**:
    *   **許可**: 「スタンプが使用可能になりました！」
    *   **拒否**: 「理由: [管理者記載の理由]」＋ 拒否通知
