...
> **⚠️ Gemini 2.5 Flash 思考機能に関する注意**
>
> Gemini 2.5 Flashは内部で「思考（reasoning）」機能を持ち、`maxOutputTokens`の一部を思考プロセスに消費します。
> 
> **問題事象**: `maxOutputTokens: 1024`設定時、約980トークンが思考に消費され、実際のコメント出力に使えるトークンが約40トークンしか残らず、文章が途中で切れる問題が発生。
>
> **対策**: `maxOutputTokens`を`4096`に増加。思考トークンは課金対象外のため、コスト影響はほとんどなし。
>
> **デバッグ方法**: Cloud Functionsログで `[GEMINI DEBUG]` を検索し、`finishReason` が `MAX_TOKENS` の場合はトークン制限が原因。
