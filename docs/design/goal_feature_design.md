...
### 9.1 概要
*   既存の目標の内容（タイトル、詳細、カラー、期限、リマインダー）を編集可能にする。
*   目標詳細画面の右上にある「ペンアイコン」から編集画面へ遷移。

### 9.2 技術仕様
*   **UI再利用**: `CreateGoalScreen` を改修し、`goal` 引数を受け取れるようにして編集モードに対応。
*   **データ更新**: `GoalService.updateGoal` を使用してFirestore上のデータを更新。
*   **自動リマインダー更新**: Cloud Functions (`scheduleGoalReminders`) が目標更新イベントを検知し、期限変更などを自動反映して通知を再スケジュールする。

## 10. 自動投稿設定 (2025-12-28)

### 10.1 概要
ユーザーはプロフィール設定画面から、自動投稿機能のON/OFFを切り替えることができる。

### 10.2 設定項目
*   **ストリーク達成時**: 連続達成（マイルストーン）時の自動投稿 (ON/OFF)
*   **目標達成時**: 目標を達成した時の自動投稿 (ON/OFF)

### 10.3 デフォルト値
*   どちらもデフォルトは **ON**。

### 10.4 挙動詳細
*   OFFの場合も、UI上の演出（紙吹雪、スナックバーの称賛メッセージ）は行われる。
*   ただし、スナックバーの文言から「投稿しました」という記述は除外される。
