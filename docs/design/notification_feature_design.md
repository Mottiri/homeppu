...
> この文書内の task/goal 通知記述は履歴資料として扱ってください。
...
## 通報機能の通知 (2026-01-02追加)

### 通報関連の通知タイプ

| 通知タイプ | 対象者 | トリガー | 説明 |
|-----------|--------|---------|------|
| `admin_report` | 管理者 | 新規通報受信時 | 新規通報を受信したことを管理者に通知 |
| `post_deleted` | 被通報ユーザー | 管理者による投稿削除時 | 投稿が削除されたことを通知 |
| `post_hidden` | 被通報ユーザー | 通報5件で自動非表示時 | 投稿が通報により非表示になったことを通知 |

### 投稿の非表示フィールド統一

投稿の表示/非表示は `isVisible` フィールドで統一管理します：
...
**補足フィールド:**
- `hiddenAt`: 非表示になった日時
- `hiddenReason`: 非表示の理由（例: "通報多数のため自動非表示"）
