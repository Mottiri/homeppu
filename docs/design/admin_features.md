...
| 通報閾値 | 5件で自動非表示化 |
| 非表示通知 | 投稿者に通知 |
| 問題なし判定 | 再表示 + 通報カウントリセット |
| データ保持 | 対処済みから1ヶ月後に削除 |
| 通報者通知 | 通報時に「審査します」メッセージ |
| 被通報者通知 | 通報時は不要（アクション実施時のみ） |

### 6.4 対処アクション

| アクション | 説明 |
|-----------|------|
| 虚偽判定 | 通報を却下（通報者の徳ポイント減少） |
| 問題なし | 再表示 + 通報カウントリセット |
| 投稿削除 | 投稿を削除（投稿者に通知） |
| ユーザーBAN | アカウントを停止（ユーザーに通知） |

### 6.5 管理者画面

#### 表示項目
- 通報された投稿/コメント内容
- 被通報者: UID、表示名、アバター、総通報回数
- 通報者: UID、表示名、通報履歴件数
- 対処ボタン
