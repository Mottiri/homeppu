...
| 9. 被通報情報表示 | プロフィール画面で累計被通報回数表示 | ✅ 実装済 |
...
管理者がユーザーの累計被通報回数をプロフィール画面で確認できる機能。

### 9.1 表示仕様

| 項目 | 内容 |
|------|------|
| 表示位置 | プロフィール画面のユーザー名直下 |
| 表示条件 | 管理者かつ被通報回数が1以上 |
| 色分け | オレンジ（1〜2回）、赤（3回以上） |

### 9.2 データソース

ユーザードキュメントの `reportCount` フィールド（通報時に自動でインクリメント）
